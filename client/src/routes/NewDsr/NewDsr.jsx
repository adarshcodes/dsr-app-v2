...
function NewDsr() {
  ...
  const fetchStatus = async () => {
    setLoading(true);
    try {
      setIsLeave(await Api("/todaystatus/", "POST"));
      setLoading(false);
    } catch (error) {
  ...
  const handlePost = async (event) => {
    try {
      ...
      (await Api("/add_dsr/", "POST").errors) ? errMsg() : verificationMsg();
  ...
  const handleDraft = async () => {
    try {
      ...
      (await Api("/add_draft/", "POST").errors) ? errMsg() : verificationMsg();
      setTimeout(closeMsg, 2500);
    } catch (error) {
  ...
  const markLeave = async () => {
    try {
      ...
      (await Api("/onleave", "POST")) ? errorMsg() : verificationMsg();
      setTimeout(closeMsg, 2500);
    } catch (error) {
  ...
  const fetchLastDsr = async () => {
    try {
      setLastDsr(await Api("/lastdsr", "POST"));
      setIsUpdated(await Api("/lastdsr", "POST").isupdated);
    } catch (error) {
      return error;
    }
  };
  ...
  const saveUpdate = async () => {
    try {
      setLastDsr(await Api("/saveupdate", "POST"));
      setMsgToShow("Updated-Dsr");
      !(await Api("/saveupdate", "POST").isupdated)
        ? errMsg()
        : verificationMsg();
      setTimeout(closeMsg, 2500);
    } catch (error) {
